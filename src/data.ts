type TimelineItem = {
  date: string;
};

type EducationItem = TimelineItem & {
  degree: string;
  location: string;
  details: string[];
};

type ExperienceItem = TimelineItem & {
  role: string;
  company: string;
  location: string;
  bullets: string[];
};

type ProjectItem = TimelineItem & {
  name: string;
  bullets: string[];
  demoUrl?: string;
  demoLabel?: string;
};

type SkillGroup = {
  category: string;
  items: string[];
};

export const portfolioData: {
  name: string;
  title: string;
  profile: string;
  contact: {
    email: string;
    phone: string;
    location: string;
    github: string;
    linkedin: string;
  };
  education: EducationItem[];
  experience: ExperienceItem[];
  projects: ProjectItem[];
  skills: SkillGroup[];
  achievements: string[];
  extracurricular: string[];
  languages: string[];
} = {
  name: "Mohamed El Nakouzi",
  title: "Software Engineer | Mechatronics Engineer",
  profile:
    "Honors and Distinction Mechatronics Engineer with experience in full-stack development, AI-enabled applications, Unity, and production deployment. Skilled in Docker, AWS, Nginx, AI agents, n8n automation, robotics, and AR systems.",
  contact: {
    email: "nakouzim2004@gmail.com",
    phone: "+961 71 617 459",
    location: "Saida, Lebanon",
    github: "https://github.com/elnakouzimohamed",
    linkedin: "https://linkedin.com/in/mohamed-el-nakouzi",
  },
  education: [
    {
      degree: "B.E. in Mechatronics Engineering, Lebanese American University",
      date: "Sep 2022 - May 2026",
      location: "Byblos, Lebanon",
      details: ["Honors Program student with Distinction; CGPA: 3.74."],
    },
  ],
  experience: [
    {
      role: "AI Software Engineer",
      company: "Globalistic Tech Offshore",
      date: "March 2025 - May 2026",
      location: "Byblos, Lebanon",
      bullets: [
        "Developed and maintained AI-enabled web applications and backend services, integrating intelligent features to automate workflows and improve user experience.",
        "Built Docker-based services and n8n automation workflows to connect APIs, streamline repetitive processes, and support production-ready solutions.",
      ],
    },
    {
      role: "Unity Developer",
      company: "Globalistic Tech Offshore",
      date: "May 2025 - Feb 2026",
      location: "Byblos, Lebanon",
      bullets: [
        "Developed AR/VR Unity applications with interactive user interfaces, real-time asset rendering, and stable 3D experiences.",
        "Optimized performance, integrated assets and plugins, and delivered builds across target devices.",
      ],
    },
    {
      role: "Frontend Developer Trainee",
      company: "CreoHub Training Program",
      date: "Feb 2025 - March 2025",
      location: "Remote",
      bullets: [
        "Built frontend interfaces using Next.js, HTML, JavaScript, CSS, Tailwind CSS, and Swiper.js.",
      ],
    },
    {
      role: "Mathematics Tutor",
      company: "Lebanese American University",
      date: "Sep 2023 - Dec 2023",
      location: "Byblos, Lebanon",
      bullets: [
        "Delivered weekly Calculus III tutoring sessions for sophomore and junior university students.",
      ],
    },
  ],
  projects: [
    {
      name: "T.A.C.T: Tactical Augmented Coaching Translator",
      demoUrl: "https://youtu.be/rNGbi9OeANU?si=NVREzw0GbWTXKD8e",
      demoLabel: "Watch Demo",
      date: "Sep 2025 - May 2026",
      bullets: [
        "Developed an AI-powered AR basketball training system that converts tactic-board drawing videos into real-time 3D player animations for clearer coach-player communication.",
        "Implemented AR features such as spatial anchoring, court calibration, and persistent court placement, while integrating Roboflow-based human detection, tracking, action verification, and pose evaluation.",
      ],
    },
    {
      name: "AURA: Augmented University Real-Time Assistant",
      date: "Sep 2025 - May 2026",
      bullets: [
        "Developed an AI-powered university assistant supporting AI Tutor chat, asynchronous lecture generation, and automated assessment generation.",
        "Built agentic AI workflows using LangChain and LangGraph, and deployed the platform using Docker, AWS, and Nginx for production server access.",
      ],
    },
    {
      name: "SAM Libraries: Internet-Based Book Store",
      date: "Sep 2024 - Dec 2024",
      bullets: [
        "Developed an online bookstore using Angular, FastAPI, and MySQL, allowing users to borrow or purchase books with category filtering and admin book management.",
        "Integrated Gemini-powered AI assistance, book image previews, and librarian service booking.",
      ],
    },
    {
      name: "Real-Time Gesture Prediction",
      date: "Jan 2025 - May 2025",
      bullets: [
        "Developed a real-time hand gesture classification system using EMG signals and machine learning for human-computer interaction.",
      ],
    },
    {
      name: "B-Bot: Self-Balancing Robot with Fuzzy Control and Neural Networks",
      demoUrl: "https://youtu.be/OxzO8reckg4?si=h26SasTzShWoitak",
      demoLabel: "Watch Demo",
      date: "Jan 2025 - May 2025",
      bullets: [
        "Designed a self-balancing two-wheeled robot integrating fuzzy logic control and neural networks for adaptive stability and motion.",
        "Used Raspberry Pi for real-time processing, an IMU for balance correction, and a camera for machine learning applications.",
      ],
    },
    {
      name: "Smart Classroom Attendance System: Logic Design Project",
      date: "Logic Design Project",
      bullets: [
        "Designed a hardware-based digital attendance system using FSMs, combinational logic, and a 5-bit synchronous up/down counter to track classroom occupancy.",
        "Built truth tables, minimized Boolean functions, simulated circuit blocks using Quartus, and implemented the design using logic ICs, switches, LEDs, 7-segment displays, and a 555-timer-based clock circuit.",
      ],
    },
  ],
  skills: [
    {
      category: "Software Development",
      items: [
        "Angular",
        "Next.js",
        "Tailwind CSS",
        "Swiper.js",
        "Streamlit",
        "Node.js",
        "FastAPI",
        "MySQL",
        "PostgreSQL",
      ],
    },
    {
      category: "AI, Automation, and Data",
      items: [
        "AI Agents",
        "Machine Learning",
        "LangChain",
        "LangGraph",
        "Gemini",
        "Roboflow",
        "n8n",
      ],
    },
    {
      category: "DevOps and Deployment",
      items: [
        "Docker",
        "AWS Deployment",
        "Nginx",
        "Production Server Configuration",
      ],
    },
    {
      category: "Engineering and Mechatronics",
      items: [
        "Unity",
        "MATLAB",
        "SOLIDWORKS",
        "Control Systems",
        "Electronics",
        "Robotics",
      ],
    },
    {
      category: "Digital Systems and Hardware",
      items: [
        "Digital Logic Design",
        "FSM Design",
        "Synchronous Counters",
        "Combinational Logic",
        "Boolean Minimization",
        "Truth Tables",
        "Quartus Simulation",
        "Electronics",
        "Circuit Analysis",
      ],
    },
    {
      category: "Embedded Systems and Robotics",
      items: [
        "Raspberry Pi",
        "Arduino",
        "IMU Sensors",
        "Motor Control",
        "Control Systems",
        "Real-Time Debugging",
        "Hardware-Software Integration",
        "Robotics",
      ],
    },
  ],
  achievements: [
    "Certified SOLIDWORKS Associate (CSWA) with a perfect score.",
    "Ranked 1st in South Lebanon and 4th nationwide in the 2022 Lebanese Baccalaureate.",
  ],
  extracurricular: [
    "IEEE AI Club and Google Developers Student Club: Participated in student technology communities focused on AI, software development, and engineering innovation.",
    "Volunteer Work at Al Reaaya: Contributed over 150 hours in data collection, organization, and distribution of food and medical equipment to support elderly individuals, orphans, people of determination, and low-income families.",
  ],
  languages: ["Arabic: C2", "English: C2"],
};
